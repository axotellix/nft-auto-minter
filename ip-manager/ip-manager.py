...
#--- [ FUNCTIONS ] ----------
 # show > FreeVPN window (put on top)
def getAppWin( app ):
...
def on_created( event ):
    ...
    if file == "change-ip.txt":
        ...
        # put > FreeVPN window on top | get > coords
        app_win = getAppWin("Free VPN")

        # define > FreeVPN window coords
        x = app_win[0]
        y = app_win[1]

        # define > select server button coords
        select_server_x = x + 460
        select_server_y = y + 90
        ...
        # define > select server button coords
        server_x = x + 200
        server_y = y + 167 + 63 * servers.index(server)
